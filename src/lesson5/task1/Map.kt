...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) names[grade] = names.getOrDefault(grade, listOf()) + name
    return names
}
...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>): Unit {
    ...
    for ((key, value) in a) if (key in b && b[key] == a[key]) keysToRemove.add(key)
    for (key in keysToRemove) a.remove(key)
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    val mapOf2 = mutableMapOf<Int, Int>()
    for (elem in list.indices) {
        if (mapOf2.contains(list[elem])) return mapOf2.getOrDefault(list[elem], 0) to elem
        mapOf2[number - list[elem]] = elem
    }
    return Pair(-1, -1)
}
